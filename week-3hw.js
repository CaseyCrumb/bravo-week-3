 ...
 */
const findMax = (numbers) => Math.max(...numbers) || 0;
//Test cases
 ...
 */
const concatenateStrings = (strings, separator) => strings.join(separator);
...
const usersWithFullName = users.map((user) => ({...user, fullName: `${user.firstName} ${user.lastName}`,})); // map to scan through 'users' array, ...user to copy all from original.
...
const products = [
  ...
  //your code here
const productsWithTotalValue = products.map((product) => ({...product, totalValue: product.value * product.quantity,}));
...
const lowQuantityFruit = produce.filter((produce) => produce.type === 'Fruit' && produce.quantity < 10);


//console.log(lowQuantityFruit);
//Expected Output:
/*
[
  { name: 'Orange', type: 'Fruit', quantity: 5 },
...
const volume = l * w * h
//console.log(`length: ${l}, width: ${w}, height: ${h} and the volume: ${volume}`);
...
const {firstName: first, lastName: last, grades: grade} = student
const averageGrade = (grade.math + grade.science + grade.english) / 3;
//console.log(averageGrade);
//console.log(first, last, grade)
//console.log(`${first} ${last}'s average grade is ${averageGrade}.`);
